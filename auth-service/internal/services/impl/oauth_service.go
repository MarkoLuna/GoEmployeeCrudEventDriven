package impl

import (
	"context"
	"fmt"
	"time"

	"github.com/MarkoLuna/GoEmployeeCrudEventDriven/common/dto"
	"github.com/MarkoLuna/GoEmployeeCrudEventDriven/common/utils"
	"github.com/go-oauth2/oauth2/v4"
	"github.com/go-oauth2/oauth2/v4/generates"
	"github.com/go-oauth2/oauth2/v4/models"
	"github.com/golang-jwt/jwt"
	"github.com/labstack/echo/v4"
)

var (
	signingKey = utils.GetEnv("OAUTH_SIGNING_KEY", "00000000")
)

type LocalOAuthServiceImpl struct {
}

func NewLocalOAuthService() *LocalOAuthServiceImpl {
	return &LocalOAuthServiceImpl{}
}

func (s *LocalOAuthServiceImpl) HandleTokenGeneration(clientId string, clientSecret string, userId string) (dto.JWTResponse, error) {
	data := &oauth2.GenerateBasic{
		Client: &models.Client{
			ID:     clientId,
			Secret: clientSecret,
		},
		UserID: userId,
		TokenInfo: &models.Token{
			AccessCreateAt:  time.Now(),
			AccessExpiresIn: time.Second * 120,
		},
	}

	gen := generates.NewJWTAccessGenerate("", []byte(signingKey), jwt.SigningMethodHS512)
	access, refresh, err := gen.Token(context.Background(), data, true)
	if err != nil {
		return dto.JWTResponse{}, err
	}

	return dto.JWTResponse{
		AccessToken:  access,
		RefreshToken: refresh,
		ExpiresIn:    120,
		Scope:        "all",
		TokenType:    "Bearer",
	}, nil
}

func (s *LocalOAuthServiceImpl) ParseToken(accessToken string) (*jwt.Token, error) {
	token, err := jwt.ParseWithClaims(accessToken, &generates.JWTAccessClaims{}, func(t *jwt.Token) (interface{}, error) {
		if _, ok := t.Method.(*jwt.SigningMethodHMAC); !ok {
			return nil, fmt.Errorf("parse error")
		}
		return []byte(signingKey), nil
	})
	if err != nil {
		return nil, err
	}
	return token, nil
}

func (s *LocalOAuthServiceImpl) GetTokenClaims(accessToken string) (map[string]string, error) {
	token, err := s.ParseToken(accessToken)
	if err != nil {
		return nil, err
	}

	claims, ok := token.Claims.(*generates.JWTAccessClaims)
	if !ok || !token.Valid {
		return nil, err
	}

	return map[string]string{
		"subject":  claims.Subject,
		"audience": claims.Audience,
		"id":       claims.Id,
		"issuer":   claims.Issuer,
	}, nil
}

func (s *LocalOAuthServiceImpl) IsAuthenticated(c echo.Context) (bool, error) {
	accessToken, _ := utils.GetBearerAuth(c.Request().Header)
	return s.IsValidToken(accessToken)
}

func (s *LocalOAuthServiceImpl) IsValidToken(accessToken string) (bool, error) {
	token, err := jwt.ParseWithClaims(accessToken, &generates.JWTAccessClaims{}, func(t *jwt.Token) (interface{}, error) {
		if _, ok := t.Method.(*jwt.SigningMethodHMAC); !ok {
			return nil, fmt.Errorf("jwt parse error")
		}
		return []byte(signingKey), nil
	})
	if err != nil {
		return false, err
	}

	_, ok2 := token.Claims.(*generates.JWTAccessClaims)
	if !ok2 || !token.Valid {
		return false, err
	}
	return true, nil
}
